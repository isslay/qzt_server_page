...
import com.qzt.common.redis.manager.CacheManager;
...
public class CacheUtil {
    ...
    public static String getCacheKey(Object id, Class<?> cls) {
        ...
        return new StringBuilder(SysConstant.CacheNamespaceEnum.DATA.value()).append(cacheName).append(":").append(id).toString();
    }
    ...
    public static String getLockKey(Object id, Class<?> cls) {
        String cacheName = getCacheKey(cls);
        return new StringBuilder(SysConstant.CacheNamespaceEnum.LOCK.value()).append(cacheName).append(":").append(id).toString();
    }
    ...
    public static String getCacheKey(Class<?> cls) {
        String cacheName = SysConstant.CACHE_KEY_MAP.get(cls);
        if (StrUtil.isBlank(cacheName)) {
            ...
            SysConstant.CACHE_KEY_MAP.put(cls, cacheName);
        }
        return cacheName;
    }
}