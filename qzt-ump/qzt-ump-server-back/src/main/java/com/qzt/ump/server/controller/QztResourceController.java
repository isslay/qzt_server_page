 ...
 */
@RestController
@RequestMapping("/back/qztResource")
@Api(value = "QztResourceController", description = "QztResourceController")
public class QztResourceController extends BaseController {

    @Autowired
    private IQztResourceService service;
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztResource entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztResource> pageModel) {
        try {
            pageModel = (PageModel<QztResource>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztResource entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztResource entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztResource entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztResource entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
