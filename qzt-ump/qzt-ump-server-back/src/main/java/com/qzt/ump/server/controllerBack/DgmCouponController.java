 ...
 */
@RestController
@RequestMapping("/back/dgmCoupon")
@Api(value = "DgmCouponController", description = "DgmCouponController")
public class DgmCouponController extends BaseController {

    @Autowired
    private IDgmCouponService service;
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            DgmCoupon entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<DgmCoupon> pageModel) {
        try {
            pageModel = (PageModel<DgmCoupon>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody DgmCoupon entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            DgmCoupon entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody DgmCoupon entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            DgmCoupon entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
    ...
    public ResultModel up(@PathVariable Long id) {
        DgmCoupon entity = service.queryById(id);
        if (entity != null) {
            entity.setStatus(0);
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
