 ...
 */
@RestController
@RequestMapping("/back/dgmGoodsCoupon")
@Api(value = "DgmGoodsCouponController", description = "DgmGoodsCouponController")
public class DgmGoodsCouponController extends BaseController {

    @Autowired
    private IDgmGoodsCouponService service;
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            DgmGoodsCoupon entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<DgmGoodsCoupon> pageModel) {
        try {
            pageModel = (PageModel<DgmGoodsCoupon>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody DgmGoodsCoupon entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            DgmGoodsCoupon entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody DgmGoodsCoupon entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            DgmGoodsCoupon entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
