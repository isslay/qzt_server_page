 ...
 */
@RestController
@RequestMapping("/back/qztAccountLog")
@Api(value = "QztAccountLogController", description = "QztAccountLogController")
public class QztAccountLogController extends BaseController {
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztAccountLog entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztAccountLog> pageModel) {
        try {
            pageModel = (PageModel<QztAccountLog>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztAccountLog entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztAccountLog entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztAccountLog entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztAccountLog entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
