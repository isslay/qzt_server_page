 ...
 */
@RestController
@RequestMapping("/back/qztBaseBank")
@Api(value = "QztBaseBankController", description = "QztBaseBankController")
public class QztBaseBankController extends BaseController {
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztBaseBank entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztBaseBank> pageModel) {
        try {
            pageModel = (PageModel<QztBaseBank>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztBaseBank entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztBaseBank entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztBaseBank entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztBaseBank entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
