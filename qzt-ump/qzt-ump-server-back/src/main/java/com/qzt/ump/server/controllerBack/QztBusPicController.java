 ...
 */
@RestController
@RequestMapping("/back/qztBusPic")
@Api(value = "QztBusPicController", description = "QztBusPicController")
public class QztBusPicController extends BaseController {

    @Autowired
    private IQztBusPicService service;
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztBusPic entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztBusPic> pageModel) {
        try {
            pageModel = (PageModel<QztBusPic>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztBusPic entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztBusPic entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztBusPic entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztBusPic entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
