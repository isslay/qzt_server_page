 ...
 */
@RestController
@RequestMapping("/back/qztOrderCommen")
@Api(value = "QztOrderCommenController", description = "QztOrderCommenController")
public class QztOrderCommenController extends BaseController {

    @Autowired
    private IQztOrderCommenService service;
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztOrderCommen entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztOrderCommen> pageModel) {
        try {
            pageModel = (PageModel<QztOrderCommen>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztOrderCommen entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztOrderCommen entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztOrderCommen entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztOrderCommen entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
