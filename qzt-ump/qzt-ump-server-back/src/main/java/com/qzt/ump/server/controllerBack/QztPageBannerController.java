 ...
 */
@RestController
@RequestMapping("/back/qztPageBanner")
@Api(value = "QztPageBannerController", description = "QztPageBannerController")
public class QztPageBannerController extends BaseController {

    @Autowired
    private IQztPageBannerService service;
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztPageBanner entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztPageBanner> pageModel) {
        try {
            pageModel = (PageModel<QztPageBanner>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztPageBanner entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztPageBanner entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztPageBanner entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztPageBanner entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
