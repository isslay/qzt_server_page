 ...
 */
@RestController
@RequestMapping("/back/qztServiceOrder")
@Api(value = "QztServiceOrderController", description = "QztServiceOrderController")
public class QztServiceOrderController extends BaseController {
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztServiceOrder entity = service.selectById(id);
            QztBusiness qztBusiness = this.qztBusinessService.queryById(entity.getBusniessId());
            if(qztBusiness!=null){
                entity.setBusniessName(qztBusiness.getBusName());
            }
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztServiceOrder> pageModel) {
        try {
            pageModel = (PageModel<QztServiceOrder>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztServiceOrder entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztServiceOrder entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztServiceOrder entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztServiceOrder entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
