 ...
 */
@RestController
@RequestMapping("/back/qztTimingLog")
@Api(value = "QztTimingLogController", description = "QztTimingLogController")
public class QztTimingLogController extends BaseController {

    @Autowired
    private IQztTimingLogService service;
    ...
    */
    ...
    public ResultModel query(@PathVariable Long id) {
        try {
            QztTimingLog entity = service.selectById(id);
    ...
    */
    ...
    public ResultModel queryListPage(@RequestBody PageModel<QztTimingLog> pageModel) {
        try {
            pageModel = (PageModel<QztTimingLog>) service.find(pageModel);
     ...
     */
    @PostMapping("/add")
    public ResultModel add(@Valid @RequestBody QztTimingLog entity) {
        try {
            entity.setCreateBy(super.getCurrentUserId());
            entity.setUpdateBy(this.getCurrentUserId());
            QztTimingLog entityback = service.add(entity);
            if (entityback == null) {
     ...
     */
    @PostMapping("/modify")
    public ResultModel modify(@RequestBody QztTimingLog entity) {
        try {
            entity.setUpdateBy(this.getCurrentUserId());
            entity.setUpdateTime(new Date());
            QztTimingLog entityback = service.modifyById(entity);
            if (entityback == null) {
                return ResultUtil.fail(SysConstant.ResultCodeEnum.INCREASE_THE_FAILURE);
            } else {
                return ResultUtil.ok();
