...
@RestController
@Log4j
@Api(value = "WebQztAccountRelogController", description = "WebQztAccountRelogController")
public class WebQztAccountRelogController {
    ...
    @PostMapping("/webapi/qztAccountRelog/getListPage")
    public Map<String, Object> getListPage(String vno, String cno , Integer pageNum, Integer pageSize, String tokenId, Long userId,int conType) {
        try {
            ...
            conditionMap.put("reType",conType);
            ...
            List<QztAccountRelog> records = pageModel.getRecords();
            List rsData = new ArrayList();
            for(QztAccountRelog qztAccountRelog:records){
                Map <String,Object> dataMap = new HashMap<>();
                dataMap.put("obj",qztAccountRelog);
                dataMap.put("changeMoney", PriceUtil.exactlyTwoDecimalPlaces(Long.parseLong(qztAccountRelog.getChangeMoney()==null?"0":qztAccountRelog.getChangeMoney().toString())));
                dataMap.put("allMoney", PriceUtil.exactlyTwoDecimalPlaces(Long.parseLong(qztAccountRelog.getReMoney().toString())));
                rsData.add(dataMap);
            }
    ...
    @GetMapping("/pubapi/qztAccountRelog/selectById/{id}")
    public Map<String, Object> selectById(@PathVariable Long id) {
        try {
            QztAccountRelog entity = this.service.selectById(id);
