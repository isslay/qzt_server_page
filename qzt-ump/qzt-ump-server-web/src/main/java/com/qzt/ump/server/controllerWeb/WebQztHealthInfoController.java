 ...
 */
@RestController
@Api(value = "WebQztHealthInfoController", description = "WebQztHealthInfoController")
public class WebQztHealthInfoController {
    ...
    @Autowired
    private IQztPageBannerService qztPageBannerService;
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "type", value = "类型（1：企业文化，2：健康课堂，3：创业平台，4商学院 5成功案例）", dataType = "int", required = true, paramType = "query")
    })
    @GetMapping("/pubapi/qztHealthInfo/getListPage")
    public Map<String, Object> getListPage(String vno, String cno, Integer pageNum, Integer pageSize,Integer type) {
        try {
            Map map = new HashMap();
            ...
            List<QztHealthInfo> records = pageModel.getRecords();
            map.put("current", pageModel.getCurrent());
            map.put("size", pageModel.getSize());
            map.put("total", pageModel.getTotal());
            map.put("data", records);
            map.put("bannerMess", this.qztPageBannerService.selectById(type));
    ...
    @GetMapping("/pubapi/qztHealthInfo/selectById/{id}")
    public Map<String, Object> selectById(@PathVariable Long id) {
        try {
            QztHealthInfo entity = this.service.selectById(id);
