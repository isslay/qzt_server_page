 ...
 */
@RestController
@Api(value = "WebQztOrderCommenController", description = "评论Api")
public class WebQztOrderCommenController {

    @Autowired
    private ReturnUtilServer returnUtil;

    @Autowired
    private IQztOrderCommenService qztOrderCommenService;
    ...
    @GetMapping("/pubapi/qztOrderCommen/getListPage")
    public Map<String, Object> getListPage(String vno, String cno, Integer pageNum, Integer pageSize) {
        try {
            ...
            pageModel = (PageModel) this.qztOrderCommenService.find(pageModel);
            List<QztOrderCommen> records = pageModel.getRecords();
    ...
    @GetMapping("/pubapi/qztOrderCommen/selectById/{id}")
    public Map<String, Object> selectById(@PathVariable Long id) {
        try {
            QztOrderCommen entity = this.qztOrderCommenService.selectById(id);
