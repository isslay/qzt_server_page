layui.config({
    ...
    pkId = "";
    deptId = "";
    urls = 'back/qztBusLog/queryListPage';
    tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'businessType', title: '业务分类（01：订单处理  10 : 宝石花寄卖订单生成 11 :宝石花寄卖购买超时 12 : 宝石花寄卖放弃购买 13 : 宝石花寄卖已付款 14 : 宝石花寄卖完成 15 : 宝石花寄卖取消 16: 宝石花寄卖系统取消 17: 宝石花寄卖已收款 50 生成预存订单  60:vip支付订单  61:数字钱包支付订单）', align: 'center', minWidth: 180, templet: '<div>{{d.businessType==""?"-":d.businessType}}</div>'},
            {field: 'operNode', title: '操作节点', align: 'center', minWidth: 180, templet: '<div>{{d.operNode==""?"-":d.operNode}}</div>'},
            {field: 'businessId', title: '业务记录ID', align: 'center', minWidth: 180, templet: '<div>{{d.businessId==""?"-":d.businessId}}</div>'},
            {field: 'sortIndex', title: '排序', align: 'center', minWidth: 180, templet: '<div>{{d.sortIndex==""?"-":d.sortIndex}}</div>'},
            {field: 'busRemark', title: '备注', align: 'center', minWidth: 180, templet: '<div>{{d.busRemark==""?"-":d.busRemark}}</div>'},
    ...
    //监听工具条
    table.on('tool(qztBusLogFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        var data = obj.data;
        pkId = data.id;
        deptId = data.deptId;
        ...
        if (layEvent === 'detail') { //查看
            pageOperation = 0;
            xadmin.open('查看', 'qztBusLogmodel.html', null, null, null);
        } else if (layEvent === 'edit') { //编辑
            pageOperation = 2;
            xadmin.open('编辑', 'qztBusLogmodel.html', null, null, null);
        } else if (layEvent === 'del') { //删除
            var pkIds = [data.id];
            layer.confirm('您确定要删除吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'back/qztBusLog/delBatchByIds',
    ...
    //监听表格复选框选择
    table.on('checkbox(qztBusLogFilter)', function (obj) {
        //obj.type == "all"//全选
        //table.cache.qztBusLogTable;//当前数据表格的id
    ...
    //添加
    form.on('submit(qztBusLogAdd)', function (obj) {
        pageOperation = 1, pkId = "";
        xadmin.open('添加', 'qztBusLogmodel.html', null, null, null);
    ...
    form.on('submit(batchDel)', function (obj) {
        var checkStatus = table.checkStatus('qztBusLogFilter');
