layui.config({
    ...
    pkId = "";
    deptId = "";
    urls = 'back/qztHealthInfo/queryListPage';
    tableIns = table.render({
        ...
        cols: [[
            ...
            // {field: 'updateBy', title: '修改人', align: 'center', minWidth: 180, templet: '<div>{{d.updateBy==""?"-":d.updateBy}}</div>'},
            {
                ...
                align: 'center',
                minWidth: 260,
                templet: '<div>{{d.infoTitle==""?"-":d.infoTitle}}</div>'
            },
            // {
            //     field: 'infoPic',
            //     title: '课堂图片',
            //     align: 'center',
            //     minWidth: 180,
            //     templet: '<div><img src="{{d.infoPic}}"></div>'
            // },
            {
                field: 'orderNum',
                title: '排序',
                ...
                edit: 'text',
                width: 90,
                templet: '<div>{{d.orderNum==""?"-":d.orderNum}}</div>'
            },
            // {field: 'infoRemark', title: '说明', align: 'center', minWidth: 180, templet: '<div>{{d.infoRemark==""?"-":d.infoRemark}}</div>'},
            // {field: 'linkGoodsId', title: '外链商品ID', align: 'center', minWidth: 180, templet: '<div>{{d.linkGoodsId==""?"-":d.linkGoodsId}}</div>'},
            {
                field: 'state', title: '状态', align: 'center', width: 90, templet: function (d) {
                if (d.state == "0") {
                    return "待上线";
                } else if (d.state == "1") {
                    return "上线";
    ...
    //监听工具条
    table.on('tool(qztHealthInfoFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        var data = obj.data;
        pkId = data.id;
        deptId = data.deptId;
        ...
        if (layEvent === 'detail') { //查看
            pageOperation = 0;
            xadmin.open('查看', 'qztHealthInfomodel.html', null, null, null);
        } else if (layEvent === 'edit') { //编辑
            pageOperation = 2;
            xadmin.open('编辑', 'qztHealthInfomodel.html', null, null, null);
        } else if (layEvent === 'del') { //删除
            var pkIds = [data.id];
            layer.confirm('您确定要删除吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'back/qztHealthInfo/delBatchByIds',
            ...
            $.ajax({
                type: 'POST',
                url: "/back/qztHealthInfo/up/" + data.id,
                success: function (data) {
                    console.log(data);
                    if (data.code == 200) {
                        layer.msg("上线成功", {icon: 1, time: 2000});
            ...
            $.ajax({
                type: 'POST',
                url: "/back/qztHealthInfo/down/" + data.id,
                success: function (data) {
                    console.log(data);
                    if (data.code == 200) {
    ...
    //监听表格复选框选择
    table.on('checkbox(qztHealthInfoFilter)', function (obj) {
        //obj.type == "all"//全选
        //table.cache.qztHealthInfoTable;//当前数据表格的id
        //obj.checked//true  false
    });

    //监听单元格编辑
    table.on('edit(qztHealthInfoFilter)', function (obj) {
        var value = obj.value //得到修改后的值
            , data = obj.data //得到所在行所有键值
            , field = obj.field; //得到字段
        // layer.msg('[ID: '+ data.id +'] ' + field + ' 字段更改为：'+ value);
        var modData = {};
        if (field == "orderNum" && value == "") {
            value = 100;
        }
        modData["id"] = data.id;
        modData[field] = value;
    ...
    //添加
    form.on('submit(qztHealthInfoAdd)', function (obj) {
        pageOperation = 1, pkId = "";
        xadmin.open('添加', 'qztHealthInfomodel.html', null, null, null);
    ...
    form.on('submit(batchDel)', function (obj) {
        var checkStatus = table.checkStatus('qztHealthInfoFilter');
