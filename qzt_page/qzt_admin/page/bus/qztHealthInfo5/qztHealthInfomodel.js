...
layui.config({
    ...
    var base = layui.base,
        ...
        submitUrl = parent.pageOperation === 1 ? "back/qztHealthInfo/add" : "back/qztHealthInfo/modify";
    ...
    if (parent.pageOperation === 1 || parent.pageOperation === 2) {
        ...
        form.verify({
            infoPic: function (value, item) { //value：表单的值、item：表单的DOM对象
                if ("" == value) {
                    return '请上传第一张图';
                }
            }
        ...
        form.on('submit(btn_submit)', function (data) {
            ...
            data.field.infoRemark = editor.getSource();
    ...
    if (parent.pageOperation === 0 || parent.pageOperation === 2) {
        ...
        $.ajax({
            type: "GET",
            url: "back/qztHealthInfo/query/" + parent.pkId,
            success: function (data) {
                if (data.code === 200) {
                    var rest = data.data;
                    //循环实体
                    for (var i in rest) {
                        ...
                        if (i == "infoPic") {
                            $("#infoPic01").attr("src", rest[i]);
                            $("#infoPic").val(rest[i]);
                        }
                        if (i == "infoPic1") {
                            $("#infoPic11").attr("src", rest[i]);
                            $("#infoPic1").val(rest[i]);
                        }
                        if (i == "infoPic2") {
                            $("#infoPic22").attr("src", rest[i]);
                            $("#infoPic2").val(rest[i]);
                        }
