layui.config({
    ...
    pkId = "";
    deptId = "";
    urls = 'back/qztPageBanner/queryListPage';
    tableIns = table.render({
        //设置表头
        cols: [[
            //{type: 'checkbox', fixed: 'left'},
            {type: 'numbers', title: '序号', fixed: 'left'},
            {field: 'typeName', title: '所属页面', align: 'center', minWidth: 180, templet: '<div>{{d.typeName==""?"-":d.typeName}}</div>'},
            {field: 'picUrl', title: 'Banner图片', align: 'center', minWidth: 180, templet: '<div><img  src="{{d.picUrl}}" width="200" height="50"></div>'},
    ...
    //监听工具条
    table.on('tool(qztPageBannerFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        var data = obj.data;
        pkId = data.id;
        deptId = data.deptId;
        ...
        if (layEvent === 'detail') { //查看
            pageOperation = 0;
            xadmin.open('查看', 'qztPageBannermodel.html', null, null, null);
        } else if (layEvent === 'edit') { //编辑
            pageOperation = 2;
            xadmin.open('编辑', 'qztPageBannermodel.html', 700, 400, null);
        } else if (layEvent === 'del') { //删除
            var pkIds = [data.id];
            layer.confirm('您确定要删除吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'back/qztPageBanner/delBatchByIds',
    ...
    //监听表格复选框选择
    table.on('checkbox(qztPageBannerFilter)', function (obj) {
        //obj.type == "all"//全选
        //table.cache.qztPageBannerTable;//当前数据表格的id
    ...
    //添加
    form.on('submit(qztPageBannerAdd)', function (obj) {
        pageOperation = 1, pkId = "";
        xadmin.open('添加', 'qztPageBannermodel.html', null, null, null);
    ...
    form.on('submit(batchDel)', function (obj) {
        var checkStatus = table.checkStatus('qztPageBannerFilter');
