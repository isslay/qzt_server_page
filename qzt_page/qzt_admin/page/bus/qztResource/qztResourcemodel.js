...
layui.config({
    ...
    var base = layui.base,
        ...
        submitUrl = parent.pageOperation === 1 ? "back/qztResource/add" : "back/qztResource/modify";
    ...
    if (parent.pageOperation === 0 || parent.pageOperation === 2) {
        ...
        $.ajax({
            type: "GET",
            url: "back/qztResource/query/" + parent.pkId,
            success: function (data) {
                if (data.code === 200) {
                    var rest = data.data;
                    //循环实体
    ...
    var uploader1 = Qiniu.uploader({
        ...
        max_file_size: '80000mb', // 最大文件体积限制
        max_retries: 1, // 上传失败最大重试次数
        dragdrop: false, // 开启可拖曳上传
        chunk_size: '4mb', // 分块上传时，每块的体积
        auto_start: true, // 选择文件后自动上传，若关闭需要自己绑定事件触发上传
        filters : [
            {title : "Video files", extensions : "mp3"}, // 限定mp3后缀上传
        ],
        init: {
    ...
    var uploader2 = Qiniu.uploader({
        ...
        auto_start: true, // 选择文件后自动上传，若关闭需要自己绑定事件触发上传
        filters : [
            {title : "Video files", extensions : "mp4"}, // 限定mp4后缀上传
        ],
        init: {
            'FilesAdded': function(up, files) {
                plupload.each(files, function(file) {
