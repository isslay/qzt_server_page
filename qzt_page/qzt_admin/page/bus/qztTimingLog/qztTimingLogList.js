layui.config({
    ...
    pkId = "";
    deptId = "";
    urls = 'back/qztTimingLog/queryListPage';
    tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'busType', title: '定时业务分类（01：优惠券）', align: 'center', minWidth: 180, templet: '<div>{{d.busType==""?"-":d.busType}}</div>'},
            {field: 'busId', title: '业务记录ID', align: 'center', minWidth: 180, templet: '<div>{{d.busId==""?"-":d.busId}}</div>'},
            {field: 'resultEnforcement', title: '执行结果（成功：Y、失败：N）', align: 'center', minWidth: 180, templet: '<div>{{d.resultEnforcement==""?"-":d.resultEnforcement}}</div>'},
    ...
    //监听工具条
    table.on('tool(qztTimingLogFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        var data = obj.data;
        pkId = data.id;
        deptId = data.deptId;
        ...
        if (layEvent === 'detail') { //查看
            pageOperation = 0;
            xadmin.open('查看', 'qztTimingLogmodel.html', null, null, null);
        } else if (layEvent === 'edit') { //编辑
            pageOperation = 2;
            xadmin.open('编辑', 'qztTimingLogmodel.html', null, null, null);
        } else if (layEvent === 'del') { //删除
            var pkIds = [data.id];
            layer.confirm('您确定要删除吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'back/qztTimingLog/delBatchByIds',
    ...
    //监听表格复选框选择
    table.on('checkbox(qztTimingLogFilter)', function (obj) {
        //obj.type == "all"//全选
        //table.cache.qztTimingLogTable;//当前数据表格的id
    ...
    //添加
    form.on('submit(qztTimingLogAdd)', function (obj) {
        pageOperation = 1, pkId = "";
        xadmin.open('添加', 'qztTimingLogmodel.html', null, null, null);
    ...
    form.on('submit(batchDel)', function (obj) {
        var checkStatus = table.checkStatus('qztTimingLogFilter');
