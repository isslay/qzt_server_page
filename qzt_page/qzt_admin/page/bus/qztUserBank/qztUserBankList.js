layui.config({
    ...
    pkId = "";
    deptId = "";
    urls = 'back/qztUserBank/queryListPage';
    tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'bankType', title: '账号类型（01银行卡 02支付宝）', align: 'center', minWidth: 180, templet: '<div>{{d.bankType==""?"-":d.bankType}}</div>'},
            {field: 'bankId', title: '银行信息表ID', align: 'center', minWidth: 180, templet: '<div>{{d.bankId==""?"-":d.bankId}}</div>'},
            {field: 'bankName', title: '银行名称', align: 'center', minWidth: 180, templet: '<div>{{d.bankName==""?"-":d.bankName}}</div>'},
            {field: 'bankPic', title: '银行logo', align: 'center', minWidth: 180, templet: '<div>{{d.bankPic==""?"-":d.bankPic}}</div>'},
            {field: 'realName', title: '持卡人姓名', align: 'center', minWidth: 180, templet: '<div>{{d.realName==""?"-":d.realName}}</div>'},
            {field: 'bindingTel', title: '绑定手机号', align: 'center', minWidth: 180, templet: '<div>{{d.bindingTel==""?"-":d.bindingTel}}</div>'},
            {field: 'cardNum', title: '银行卡号', align: 'center', minWidth: 180, templet: '<div>{{d.cardNum==""?"-":d.cardNum}}</div>'},
            {field: 'bankBranchName', title: '开户行信息', align: 'center', minWidth: 180, templet: '<div>{{d.bankBranchName==""?"-":d.bankBranchName}}</div>'},
            {field: 'isDefault', title: '是否默认地址', align: 'center', minWidth: 180, templet: '<div>{{d.isDefault==""?"-":d.isDefault}}</div>'},
            {field: 'isDel', title: '是否删除', align: 'center', minWidth: 180, templet: '<div>{{d.isDel==""?"-":d.isDel}}</div>'},
    ...
    //监听工具条
    table.on('tool(qztUserBankFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        var data = obj.data;
        pkId = data.id;
        deptId = data.deptId;
        ...
        if (layEvent === 'detail') { //查看
            pageOperation = 0;
            xadmin.open('查看', 'qztUserBankmodel.html', null, null, null);
        } else if (layEvent === 'edit') { //编辑
            pageOperation = 2;
            xadmin.open('编辑', 'qztUserBankmodel.html', null, null, null);
        } else if (layEvent === 'del') { //删除
            var pkIds = [data.id];
            layer.confirm('您确定要删除吗？', {icon: 3, title: '确认'}, function () {
                $.ajax({
                    type: 'POST',
                    url: 'back/qztUserBank/delBatchByIds',
    ...
    //监听表格复选框选择
    table.on('checkbox(qztUserBankFilter)', function (obj) {
        //obj.type == "all"//全选
        //table.cache.qztUserBankTable;//当前数据表格的id
    ...
    //添加
    form.on('submit(qztUserBankAdd)', function (obj) {
        pageOperation = 1, pkId = "";
        xadmin.open('添加', 'qztUserBankmodel.html', null, null, null);
    ...
    form.on('submit(batchDel)', function (obj) {
        var checkStatus = table.checkStatus('qztUserBankFilter');
