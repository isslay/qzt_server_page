layui.config({
    ...
    tableIns = table.render({
        ...
        cols: [[
            {type: 'checkbox', fixed: 'left'},
            {
                ...
                event: 'editColumn',
                minWidth: 120,
                templet: '<div>{{d.paramKey==""?"-":d.paramKey}}</div>'
            },
            {
                field: 'paramValue',
                title: '参数值',
                ...
                event: 'editColumn',
                minWidth: 120,
                templet: '<div>{{d.paramValue==""?"-":d.paramValue}}</div>'
            },
            {field: 'remark', title: '备注', align: 'center', edit: 'text', event: 'editColumn', minWidth: 120, templet: '<div>{{d.remark==""?"-":d.remark}}</div>'},
            {field: 'enable', title: '启用状态', align: 'center', templet: '#switchTpl', event: 'editColumn', minWidth: 120},
    ...
    table.on('tool(tableFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            xadmin.open('查看', 'param.html', 600, 400, null);
            ...
            pageOperation = 2;
            checkedparamId = data.id;
            xadmin.open('编辑', 'param.html', 600, 400, null);
        }
    });
    ...
    //更新redis
    form.on('submit(redisUpdate)', function (obj) {
        var indexMsg = layer.msg('更新中，请稍候', {icon: 16, time: false, shade: 0.8});
        $.ajax({
            type: 'POST',
            url: 'param/updateRedis',
            success: function (data) {
                if (data.code === 200) {
    ...
    //添加参数按钮监听
    form.on('submit(paramAdd)', function (obj) {
        pageOperation = 1;
        xadmin.open('添加', 'param.html', 600, 400, null);
        //阻止表单跳转。如果需要表单跳转，去掉这段即可。
        return false;
    });
